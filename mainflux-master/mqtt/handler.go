...
// NewHandler creates new Handler entity
func NewHandler2(publishers []messaging.Publisher, es redis.EventStore,
...
func (h *handler) Publish(c *session.Client, topic *string, payload *[]byte) {
	...
	fmt.Println("Publish - client ID " + c.ID + " to the topic: " + *topic)
	//fmt.Println("Subscribe - client ID: " + c.ID + ", to topics: " + strings.Join(*topics, ","))
